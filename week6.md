 ...
 ***背包问题在循环时一般第一层枚举物品，第二层枚举重量，第三层枚举决策数，并且如果是优化到1维则循环顺序不能变***
...
## 线性DP
### 数字三角形
f(i, j)表示所有从起点走到(i, j)的路径
最后需要求属性是f(i, j)的最大值
状态计算：
有两种状态 对于f(i, j)可分为
**来自左上`f(i - 1, j - 1) + a[i, j]`**
**来自右上`f(i - 1, j) + a[i, j]`**
***注意初始化时边界的问题，需要多初始化两列为-INF***
```C++
#include <iostream>
using namespace std;
...
表示的集合：所有以第i个数结尾的上升子序列

要求的属性：集合的最大值

状态计算：
划分为i类：
第0类表示只有a\[i\]一个数 上升子序列为1

**状态转移方程：**`f(i)=max(f(j) + 1), j=0,1,2……,i-1`

j枚举的时i前面i-1个数

如果a\[j\]比a\[i\]小则a\[j\]可以作为最长上升子序列中的一个元素
```C++
#include <iostream>
...
int main() {
    ...
    for (int i = 1; i <= n; i ++) {
        f[i] = 1; // 其中一种情况 只有a[i]一个数
        for (int j = 1; j < i; j ++) // 用j枚举i前面的数
            if (a[j] < a[i]) { // 前面的数如果小于a[i]
                f[i] = max(f[i], f[j] + 1); // f[j]已经求出
            }
    }
    
    int ans = 0;
    
    for (int i = 1; i <= n; i ++) ans = max(ans, f[i]); // 遍历集合求最大值
